export const BlueTheme = {
  MainBackground: 'hsl(222, 26%, 31%)',
  ToggleBackground: ' hsl(223, 31%, 20%)',
  ToggleBall: 'hsl(6, 63%, 50%)',
  KeypadBackground: 'hsl(223, 31%, 20%)',
  ScreenBackground: 'hsl(224, 36%, 15%)',
  TitleColor: 'hsl(0, 0%, 100%)',

  PrimaryKeyColor: 'hsl(221, 14%, 31%)',
  PrimaryKeyBackground: 'hsl(30, 25%, 89%)',
  PrimaryKeyShadow: 'hsl(28, 16%, 65%)',

  SingularkeyColor: 'hsl(0, 0%, 100%)',
  SingularkeyBackground: 'hsl(6, 63%, 50%)',
  SingularkeyShadow: ' hsl(6, 70%, 34%)',

  SecondarykeyColor: 'hsl(0, 0%, 100%)',
  SecondarykeyBackground: 'hsl(225, 21%, 49%)',
  SecondarykeyShadow: 'hsl(224, 28%, 35%)',
}

export const GrayTheme = {
  MainBackground: 'hsl(0, 0%, 90%)',
  ToggleBackground: 'hsl(0, 5%, 81%)',
  ToggleBall: 'hsl(25, 98%, 40%)',
  KeypadBackground: 'hsl(0, 5%, 81%)',
  ScreenBackground: 'hsl(0, 0%, 93%)',
  TitleColor: 'hsl(60, 10%, 19%)',

  PrimaryKeyColor: 'hsl(60, 10%, 19%)',
  PrimaryKeyBackground: 'hsl(45, 7%, 89%)',
  PrimaryKeyShadow: 'hsl(35, 11%, 61%)',

  SingularkeyColor: 'hsl(0, 0%, 100%)',
  SingularkeyBackground: 'hsl(25, 98%, 40%)',
  SingularkeyShadow: 'hsl(25, 99%, 27%)',

  SecondarykeyColor: 'hsl(0, 0%, 100%)',
  SecondarykeyBackground: 'hsl(185, 42%, 37%)',
  SecondarykeyShadow: 'hsl(185, 58%, 25%)',
}

export const PurpleTheme = {
  MainBackground: 'hsl(268, 75%, 9%)',
  ToggleBackground: 'hsl(268, 71%, 12%)',
  ToggleBall: 'hsl(176, 100%, 44%)',
  KeypadBackground: 'hsl(268, 71%, 12%)',
  ScreenBackground: 'hsl(268, 71%, 12%)',
  TitleColor: 'hsl(52, 100%, 62%)',

  PrimaryKeyColor: 'hsl(52, 100%, 62%)',
  PrimaryKeyBackground: 'hsl(268, 47%, 21%)',
  PrimaryKeyShadow: 'hsl(290, 70%, 36%)',

  SingularkeyColor: 'hsl(198, 20%, 13%)',
  SingularkeyBackground: 'hsl(176, 100%, 44%)',
  SingularkeyShadow: 'hsl(177, 92%, 70%)',

  SecondarykeyColor: 'hsl(0, 0%, 100%)',
  SecondarykeyBackground: 'hsl(281, 89%, 26%)',
  SecondarykeyShadow: 'hsl(285, 91%, 52%)',
}
